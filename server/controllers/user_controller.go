...
func CreateUser(c echo.Context) error {
	...
	if user.ClerkId == "" {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{
			...
			Message: "error",
			Data:    &echo.Map{"data": "missing ClerkId"},
		})
	}

	// Check if user exists in db
	var existingUser models.User
	err := userCollection.FindOne(ctx, bson.M{"clerkid": user.ClerkId}).Decode(&existingUser)

	if err == nil {
		return c.JSON(http.StatusOK, responses.UserResponse{
			Status:  http.StatusOK,
			Message: "user already exists",
			Data:    &echo.Map{"data": existingUser},
		})
	}
...
func GetUserByClerkId(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	clerkId := c.Param("clerkid")
	var user models.User
	defer cancel()

	err := userCollection.FindOne(ctx, bson.M{"clerkid": clerkId}).Decode(&user)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, responses.UserResponse{Status: http.StatusInternalServerError, Message: "error", Data: &echo.Map{"data": err.Error()}})
	}
...
func EditUser(c echo.Context) error {
	...
	// update := bson.M{"username": user.Username, "location": user.Location, "title": user.Title}
	update := bson.M{"Email": user.Email, "username": user.Username, "ClerkId": user.ClerkId}
